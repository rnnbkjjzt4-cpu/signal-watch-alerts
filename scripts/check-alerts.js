...
function saveJSON(p, data) {
  fs.mkdirSync(path.dirname(p), { recursive: true }); // ensure state/ exists even on a fresh checkout
...
async function main() {
  ...
  // Cap per run so a big backlog (e.g. first run ever, or a slow news day) trickles
  // out over several 15-min cycles instead of blasting Gmail and getting throttled.
  ...
  if (!GMAIL_USER || !GMAIL_APP_PASSWORD) {
    ...
    // pool + low rateLimit reuses one authenticated connection instead of
    // logging in fresh for every message, which is what tripped Gmail's
    // "Too many login attempts" throttle on the first run.
  ...
  if (eventsDeferred.length > 0) {
    console.log(`${eventsDeferred.length} events deferred to next run to avoid a send burst.`);
  }
